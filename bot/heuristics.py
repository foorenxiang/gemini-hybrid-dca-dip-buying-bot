...
# # TODO: add logic to handle condition where there are already existing low limit order prices placed when filling in gap
...
# # TODO: pin to a certain price multiple
# # TODO: add logic to handle gap between highest limit order price being much lower than current market order prices
...
def _simple_should_create_limit_orders_heuristic() -> Optional[Tuple[float]]:
    ...
    available_balance_for_limit_orders = (
        tkn_b_account_balance - config.reserved_amount_for_market_orders
    )
